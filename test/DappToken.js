    ...
    it('approves tokens for delegated transfer', function() {
        return DappToken.deployed().then(function(instance) {
          ...
          return tokenInstance.allowance(accounts[0], accounts[1]);
            ...
            //console.log(await allowance.toNumber());
      ...
      it('handles delegated token transfers', function() {
        return DappToken.deployed().then(async function(instance) {
        ...
        }).then(function(balacke){
          //console.log("accounts to ka balance" , balacke.toNumber());
          return tokenInstance.transfer(fromAccount, 100, { from: accounts[0] });
        }).then(function(receipt) {
          // Approve spendingAccount to spend 50 tokens from fromAccount
          return tokenInstance.approve(spendingAccount, 50, { from: fromAccount });
        }).then(function(receipt) {
          // Try transferring something larger than the sender's balance
          return tokenInstance.transferFrom(fromAccount, toAccount, 9999999, { from: spendingAccount });
        }).then(assert.fail).catch(function(error) {
          console.log("value larger than balance" , error.message);
          assert(error.message.indexOf('revert') >= 0, 'cannot transfer value larger than balance');
          // Try transferring something larger than the approved amount
          return tokenInstance.transferFrom(fromAccount, toAccount, 999, { from: spendingAccount });
        }).then(assert.fail).catch(function(error) {
            console.log("cannot transfer value larger than approved amount", error.message);
          ...
          //console.log("fromaccount balance" , balance.toNumber());
          assert.equal(balance.toNumber(), 90, 'deducts the amount from the sending account');
          return tokenInstance.balanceOf(toAccount);
        }).then(function(balance) {
         // console.log("to account balance" , balance.toNumber());
          assert.equal(balance.toNumber(), 10, 'adds the amount from the receiving account');
          return tokenInstance.allowance(fromAccount, spendingAccount);
        }).then(function(allowance) {
          //console.log(allowance.toNumber());
          assert.equal(allowance.toNumber(), 40, 'deducts the amount from the allowance');
        });
      });
})